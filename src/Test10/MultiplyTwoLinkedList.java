...
public class MultiplyTwoLinkedList {
    /*
    1. You are given two linked lists. Data of each node of these linked lists
    contain a digit from the range: [0, 9].
    2. The linked lists represent two numbers.
    3. You have to print the product of these two numbers.

    Input Format:

    Input has already been managed for cpp and java submissions. In cpp and java,
    you have to just write a function which receives head nodes of two linked lists.
    The sizes of linked lists are N and M, respectively.
    ...
    public static LinkedListNode<Integer> reverse(LinkedListNode<Integer> head){
       ...
       LinkedListNode<Integer> tail = reverse(head.next);
       head.next.next = head;
       head.next = null;
       return tail;
        ...
        for (int i = 0; i <len1 ; i++) {
           ...
           add = result;
            ...
            for (int j = 0; j < len2; j++) {
                ...
                extra = num/10;
                if(add.next==null && (j<len2-1||extra!=0)) add.next = new LinkedListNode<>(0);
                add = add.next;
                temp2 = temp2.next;
            }
